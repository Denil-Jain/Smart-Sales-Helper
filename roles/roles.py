...
def add():
    ...
    if form.validate_on_submit():
        try:
            result = DB.insertOne("INSERT INTO SMART_Roles (name, description, is_active) VALUES (%s, %s, %s)",
...
def edit():
    ...
    if form.validate_on_submit() and id:
        try:
            result = DB.insertOne("UPDATE SMART_Roles set name = %s, description = %s, is_active = %s WHERE id = %s",
    ...
    try:
        result = DB.selectOne("SELECT name, description, is_active from SMART_Roles WHERE id = %s", id)
...
def list():
    rows = [] 
    try:
        result = DB.selectAll("SELECT id,name, description, is_active FROM SMART_Roles LIMIT 100",)
...
def delete():
    ...
    if id:
        try:
            result = DB.delete("DELETE FROM SMART_Roles WHERE id = %s", id)
            if result.status:
    ...
    if email:
        try:
            ...
            SELECT id, email, 
                ...
                SMART_UserRoles ur JOIN SMART_Roles on ur.role_id = SMART_Roles.id WHERE ur.user_id = SMART_Users.id) as roles
            FROM SMART_Users where email like %s limit 10
            ...
            flash(str(e), "danger")
    result = DB.selectAll("SELECT id, name FROM SMART_Roles WHERE is_active = 1",)
    if result.status and result.rows:
        roles = result.rows
...
def apply():
    ...
    if users and roles: # we need both for this to work
        ...
        if len(mappings) > 0:
            try:
                result = DB.insertMany("INSERT INTO SMART_UserRoles (user_id, role_id, is_active) VALUES(%s, %s, 1) ON DUPLICATE KEY UPDATE is_active = !is_active", mappings)
                if result.status:
