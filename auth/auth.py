...
def check_duplicate(e):
    ...
    import re
    r = re.match(".*SMART_Users.(\w+)", e.args[0].args[1])
    if r:
...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            result = DB.insertOne("INSERT INTO SMART_Users (email, username, password) VALUES (%s, %s, %s)", email, username, hash)
            if result.status:
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if is_valid:
            try:
                result = DB.selectOne("SELECT id, email, username, password FROM SMART_Users where email= %(email)s or username=%(email)s", {"email":email})
                if result.status and result.row:
                    ...
                    if bcrypt.check_password_hash(hash, password):
                        ...
                        # get roles
                        result = DB.selectAll("""
                        SELECT name FROM SMART_Roles r JOIN SMART_UserRoles ur on r.id = ur.role_id WHERE ur.user_id = %s AND r.is_active = 1 AND ur.is_active = 1
                        """, user.id)
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        if current_password and password and confirm:
            try:
                result = DB.selectOne("SELECT password FROM SMART_Users where id = %s", user_id)
                if result.status and result.row:
                    ...
                    if bcrypt.check_password_hash(result.row["password"], current_password):
                        ...
                        hash = bcrypt.generate_password_hash(password)
                        try:
                            result = DB.update("UPDATE SMART_Users SET password = %s WHERE id = %s", hash, user_id)
                            if result.status:
        ...
        if is_valid:
            try: # update email, username (this will trigger if nothing changed but it's fine)
                result = DB.update("UPDATE SMART_Users SET email = %s, username = %s WHERE id = %s", email, username, user_id)
                if result.status:
    ...
    try:
        ...
        result = DB.selectOne("SELECT id, email, username FROM SMART_Users where id = %s", user_id)
        if result.status and result.row:
            user = User(**result.row)
